...
bool CMAES::TestForTermination()
  {
    ...
    {
      std::cout << "Fitness: function value " << functionValues[(int)index[0]]
    ...
    if (cTemp == 2*N)
    {
       std::cout << "TolX: object variable changes below "
       << stopTolX << std::endl;
       return true;
    }
    ...
    {
       std::cout << "ConditionNumber: maximal condition number " <<
...
void CMAES::UpdateEigenSystem()
  {
    Update();
      ...
      if (eigensysIsUptodate)
      {
        std::cout << " eigen system return " << std::endl;
        return;
      }
      ...
      {
        std::cout << " modul gen return " << std::endl;
        return;
      }
